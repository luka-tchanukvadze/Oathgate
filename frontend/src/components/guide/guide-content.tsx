...
// Rendered on two routes: inside the dashboard, and on a public page a visitor
// can read before deciding to open a workspace. The screen list is the only
// part that differs, because those links go somewhere a visitor cannot reach
export function GuideContent({ signedIn = true }: { signedIn?: boolean }) {
  return (
    <>
      ...
      <Panel className="mt-4">
        <PanelHeader>
          <PanelTitle>{signedIn ? 'About this workspace' : 'About the demo workspace'}</PanelTitle>
        </PanelHeader>
        <PanelBody className="space-y-3 text-sm leading-relaxed text-ink-muted">
          <p>
            {signedIn ? 'You are signed in to' : 'Opening the demo gives you'} a workspace of your own,
            created the moment you click in and named after a shop picked at random. No email, no
            password, and nobody else&rsquo;s payments in it. It is seeded with two weeks of activity so
      ...
      <Panel className="mt-4">
        ...
        <ul className="divide-y divide-line">
          {SCREENS.map(({ href, name, what }) => (
            <li key={href}>
              {/* Plain rows for a visitor, because every one of these links
                  lands on a login screen until they have a workspace */}
              {signedIn ? (
                <Link
                  href={href}
                  className="flex items-center gap-4 px-5 py-3.5 hover:bg-surface-muted sm:px-6"
                >
