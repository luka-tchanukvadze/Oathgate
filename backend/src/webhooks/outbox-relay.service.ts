...
export class OutboxRelayService {
  ...
  async relay(): Promise<void> {
    try {
      const deliveryIds = await this.claimBatch();
      ...
      await this.enqueue(deliveryIds);
    } catch (error) {
  ...
  private async claimBatch(): Promise<string[]> {
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const event of events) {
        deliveryIds.push(...(await this.fanOut(tx, event)));
      }
  ...
  private async fanOut(tx: Tx, event: OutboxEvent): Promise<string[]> {
  ...
  private async enqueue(deliveryIds: string[]): Promise<void> {
    ...
    await Promise.race([
      this.queue.addBulk(
        deliveryIds.map((deliveryId) => ({
          name: WEBHOOK_JOB,
          data: { deliveryId },
          // The delivery row's own id. Redis then refuses a duplicate, so a
          // replayed batch cannot put the same delivery in the queue twice
          opts: { jobId: deliveryId },
        })),
      ),
    ...
    this.logger.log(`queued ${deliveryIds.length} webhook deliveries`);
  }
}
