...
export class EventPublisher implements OnModuleDestroy {
  ...
  constructor(config: ConfigService) {
    ...
    // I override the queue's retry policy here
    // A worker can wait for ever on a blocking read
    // This runs on a 5 second clock and has to fail fast
  ...
  // Best effort, and that is the thing to remember
  // A message sent with no subscriber is gone, not queued
  // Webhooks survive an outage because they are rows in Postgres
  async publish(events: DomainEvent[]): Promise<void> {
    ...
    try {
      ...
      // exec never throws, it hands back an [error, result] per command
      // Without this a publish that failed on every event still logged success
      ...
      // Swallowed so a pub/sub problem never stops a webhook
      // Same relay, deliberately not the same fate
